...
From there, the merchant can extract the <code>transID</code> value and use it to call the Result API to get the result of the 3DS challenge, and retrieve the <code>eci</code> and <code>cavv</code> values required for the sale operation. 
...
The merchant must use the <code>redirectUrl</code> value to [setup a hidden iframe](v2-create-iframe.md), and listen for a Post Message Event. If successful, proceed to call the Auth API.
